    ...
    <section className="report" aria-labelledby="report-title">
      <div className="portal-container home-section">
        <div className="section-heading">
          ...
          <p className="report__guard"><ShieldCheck size={15} /> 신고자 정보는 관련 법령에 따라 보호됩니다.</p>
        </div>
          ...
          {reportServices.map((service) => {
            ...
            return (
              <article className="report-card" key={service.title}>
                <span className={`report-card__icon report-card__icon--${service.tone}`}><Icon size={21} strokeWidth={1.8} /></span>
                <h3>{service.title}</h3>
